...
def get_random_number():
    return(random.randint(MIN_RAND_NBR,MAX_RAND_NBR))
...
def read_results(quiz_id):
    ...
    for row in csvreader:
        if row[1] == START_QUIZ and str(row[2]) == quiz_id:
            quiz_name = row[3]
            quiz_date = row[0][:8]
        if row[1] == PARTICIPANT and str(row[2]) == quiz_id:
            ip_address = row[3]
        ...
        if row[1] == ANSWER and str(row[2]) == quiz_id:
            ip_address = row[3]
            participant_id = str(row[4])
        ...
        if row[1] == STATUS and str(row[2]) == quiz_id:
            participant_id = str(row[4])
            key = " ".join([participant_id])
            results[key]["status"] = row[5]
    infile.close()
